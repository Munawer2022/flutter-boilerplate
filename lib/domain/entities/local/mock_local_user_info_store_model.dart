...
class MockLocalUserInfoStoreModel extends Equatable {
  const MockLocalUserInfoStoreModel({required this.token});

  final String token;

  MockLocalUserInfoStoreModel copyWith({String? token}) {
    return MockLocalUserInfoStoreModel(token: token ?? this.token);
  }

  factory MockLocalUserInfoStoreModel.fromJson(Map<String, dynamic> json) =>
      MockLocalUserInfoStoreModel(token: json["token"] ?? "");
